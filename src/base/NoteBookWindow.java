...
public class NoteBookWindow extends Application {
	...
	/**
	 * absolute path of Notebook object
	 */
	String noteBookPath = "";
	/**
		...
		buttonAddFolder.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
			    ...
			    if (result.isPresent()){
			    	...
			    	for (String folderName : foldersComboBox.getItems()) {
			    		if (folderName.equals(result.get())) {
			    			Alert alert = new Alert(AlertType.WARNING);
		...
		for (Folder f : noteBook.getFolders()) {
			if (f.getName().equals(currentFolder)) {
				if (currentSearch.equals("")) {
					...
					List<Note> result = f.searchNotes(currentSearch);
	...
	private void loadNoteBook(File file) {
		noteBookPath = file.getAbsolutePath();
		noteBook = new NoteBook(noteBookPath);
	}
